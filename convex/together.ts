...
export const chat = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
        ...
        messages: [
          {
            role: 'system',
            content:
              'Le texte suivant est la transcription d\'un message vocal. Extraire un titre, un résumé et des tâches à faire. Répondre en JSON avec ce format : {title: string, summary: string, actionItems: [string, string, ...]}. Le résumé doit être à la première personne du point de vue de la personne qui enregistre le message.',
          },
      ...
      console.error('Erreur lors de l\'analyse du message vocal', e);
      await ctx.runMutation(internal.together.saveSummary, {
        id: args.id,
        summary: 'Le résumé n\'a pas pu être généré. Veuillez réessayer.',
        actionItems: [],
...
export const saveSummary = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!note) {
      console.error(`Note introuvable ${id}`);
      return;
    }
